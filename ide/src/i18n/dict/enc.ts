...
export const dict: Record<string, { ko: string; en: string; de: string; ja: string }> = {
  ...
  // 찾아 바꾸기가 건너뛴 파일 — 조용히 빼면 "전부 바꿨다" 로 읽힌다.
  "enc.replaceSkipped": { ko: "UTF-8 이 아니라 건드리지 않은 파일 {n}개: {files}", en: "{n} file(s) left untouched because they are not UTF-8: {files}", de: "{n} Datei(en) unberührt gelassen, da nicht UTF-8: {files}", ja: "UTF-8 ではないため触れなかったファイル {n} 件: {files}" },
